<template>
    <div class="flex justify-start items-center lg:justify-center lg:items-center overflow-x-scroll">
        <table class="border-separate">
            <thead class="">
                <tr>
                    <th class="w-36 min-w-[144px] border">Nom</th>
                    <th @click="sortByPrice($store.state.listManga)" class="w-36 min-w-[144px] border cursor-pointer">Prix <span v-show="sort == true" class="inline-block transform -rotate-90">></span><span v-show="sort == false" class="inline-block transform rotate-90">></span></th>
                    <th class="w-36 min-w-[144px] border">Parution</th>
                    <th @click="sortByLastBoughtVolume($store.state.listManga)" class="w-36 min-w-[144px] border">Tomes Sortis</th>
            ...
            <tbody v-if="$store.state.listManga.length > 0">
                <tr v-for="(manga, index) in $store.state.listManga" :key="manga.id" class="hover:bg-gray-400 h-12">
                    ...
                    <td class="w-36 min-w-[144px] border">{{ priceFormat(manga.price) }}</td>
